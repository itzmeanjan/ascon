...
// Compile it with:
// `dpcpp -std=c++20 -fsycl -O3 -I ./include example/accel_ascon_128a.cpp`
int
main()
{
  ...
  // these many instances of Ascon-128a to be invoked
  constexpr size_t wi_cnt = 1024ul;
  // these many Ascon-128a invocation instances to be grouped together
  constexpr size_t wg_size = 32ul;
  // each Ascon-128a invocation to encrypt 64 -bytes of input text
  constexpr size_t per_wi_ct_len = 64ul;
  // each Ascon-128a invocation to consume 32 -bytes of associated data
  // while performing authenticated encryption/ verified decryption
  //
  ...
  //
  // note, it must be true for all Ascon-128a instances !
  ...
  // check that verification flag is true for all Ascon-128a instances ( check
  ...
  std::cout << "Accelerated Ascon-128a works !" << std::endl;
