...
// Compile it with:
//
// dpcpp -std=c++20 -fsycl -O3 -I ./include example/accel_ascon_80pq.cpp
int
main()
{
  ...
  // these many instances of Ascon-80pq to be invoked
  constexpr size_t wi_cnt = 1024ul;
  // these many Ascon-80pq invocation instances to be grouped together
  constexpr size_t wg_size = 32ul;
  // each Ascon-80pq invocation to encrypt 64 -bytes of input text
  constexpr size_t per_wi_ct_len = 64ul;
  // each Ascon-80pq invocation to consume 32 -bytes of associated data
  ...
  // each work-item to consume secret keys of 160 -bit
  constexpr size_t per_wi_k_len = 20ul;
  // public message nonce & authentication tag ( generated after
  // authenticated encryption process ) --- each are of 128 -bits, to be used/
  // consumed by each work-item
  constexpr size_t per_wi_nt_len = sizeof(uint64_t) << 1;
  // each work-item to generate single boolean flag denoting status of
  ...
  // total bytes for keeping secret key ( for all work-items )
  constexpr size_t k_len = wi_cnt * per_wi_k_len;
  // total bytes for keeping public message nonce/ authentication
  // tag ( for all work-items )
  constexpr size_t nt_len = wi_cnt * per_wi_nt_len;
  ...
  // secret keys
  uint8_t* keys = static_cast<uint8_t*>(sycl::malloc_shared(k_len, q));
  // public message nonces
  uint8_t* nonces = static_cast<uint8_t*>(sycl::malloc_shared(nt_len, q));
  // authentication tags
  uint8_t* tags = static_cast<uint8_t*>(sycl::malloc_shared(nt_len, q));
  // verification flags
  ...
  ascon_utils::random_data(keys, k_len);
  // prepare random public message nonces
  ascon_utils::random_data(nonces, nt_len);

  using evt = sycl::event;

  evt e0 = q.memset(enc, 0, ct_len);
  evt e1 = q.memset(tags, 0, nt_len);

  // generate encrypted bytes & authentication tags
  evt e2 = accel_ascon::encrypt_80pq(q,
                                     keys,
                                     k_len,
                                     nonces,
                                     nt_len,
  ...
  evt e5 = accel_ascon::decrypt_80pq(q,
                                     keys,
                                     k_len,
                                     nonces,
                                     nt_len,
  ...
  // check that verification flag is true for all Ascon-80pq instances ( check
  // it sequentially on host )
  ...
  std::cout << "Accelerated Ascon-80pq works !" << std::endl;
