...
// Compile it with:
// `dpcpp -std=c++20 -fsycl -O3 -I ./include example/accel_ascon_hash.cpp`
int
main()
{
  ...
  // these many Ascon-Hash digests to be computed in-parallel
  constexpr size_t wi_cnt = 1024ul;
  // these many work-items to be grouped into single work-group
  constexpr size_t wg_size = 32ul;
  // each work-item will compute Ascon-Hash digest on 64 input bytes
  constexpr size_t per_wi_msg_len = 64ul;
  // each work-item will produce Ascon-Hash digest of 32 -bytes
  ...
  // data-parallelly compute Ascon-Hash digests for `wi_cnt` -many independent,
  // non-overlapping input byte sequences & for each of them contiguously place
  // 32 digest bytes in respective memory locations
  evts e1{ e0 };
  evt e2 = accel_ascon::hash(q, msg, i_len, dig, o_len, wi_cnt, wg_size, e1);
  ...
  for (size_t wi = 0; wi < wi_cnt; wi++) {
    ...
    // compute Ascon-Hash digest on single text byte slice;
    // do it for `wi_cnt` -many times !
    ascon::hash(msg + i_off, per_wi_msg_len, dig_);
  ...
  std::cout << "Accelerated Ascon-Hash works !" << std::endl;
