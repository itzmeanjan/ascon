...
def xofa(msg: bytes, olen: int) -> bytes:
    """
    Computes olen -bytes Ascon XOFA of arbitrary length input byte array;
    see section 2.5 of Ascon specification
    https://ascon.iaik.tugraz.at/files/asconv12-nist.pdf
    """
    digest = create_string_buffer(olen)

    SO_LIB.xofa.argtypes = [c_char_p, c_size_t, c_char_p, c_size_t]
    SO_LIB.xofa(msg, len(msg), digest, olen)
