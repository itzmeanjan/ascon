...
def encrypt_128(key: bytes, nonce: bytes, data: np.ndarray, text: np.ndarray) -> Tuple[np.ndarray, bytes]:
    ...
    args = [secret_key_128_tp, nonce_tp,
            data_t, len_t, text_t, len_t, cipher_t]
...
def encrypt_80pq(key: bytes, nonce: bytes, data: np.ndarray, text: np.ndarray) -> Tuple[np.ndarray, bytes]:
    '''
    ...
    args = [secret_key_160_tp, nonce_tp,
            data_t, len_t, text_t, len_t, cipher_t]
...
def decrypt_80pq(key: bytes, nonce: bytes, data: np.ndarray, cipher: np.ndarray, tag: bytes) -> Tuple[bool, np.ndarray]:
    '''
    ...
    args = [secret_key_160_tp, nonce_tp, data_t,
            len_t, cipher_t, len_t, text_t, tag_tp]
    ...
    v = SO_LIB.decrypt_80pq(key_, nonce_, data, d_len,
                           cipher, c_len, text, tag_)
