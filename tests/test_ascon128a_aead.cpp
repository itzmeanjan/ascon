...
)
{
  ...
  bool flag = ascon128a_aead::decrypt(_key, _nonce, _data, _enc, _dec, _tag);
...
kat_ascon128a_aead()
{
  ...
  while (true) {
    ...
    if (!std::getline(file, count0).eof()) {
      ...
      ascon128a_aead::encrypt(_key, _nonce, _ad, _pt, _ctxt, _tag);
      bool flag = ascon128a_aead::decrypt(_key, _nonce, _ad, _ctxt, _ptxt, _tag);
